...
// Type: dqgToPdf.Dwg.R2013.Objects.MLeader
...
{
  public class MLeader : CommonEntityData
  {
    ...
    private void loadMLeaderData(DwgBitArray ba)
    {
      ...
      this.AttachmentDirection = (int) ba.ReadBS();
      Console.WriteLine(string.Format("AttachmentDirection: {0}", (object) this.AttachmentDirection));
      this.StyleTopTextAttachment = (int) ba.ReadBS();
      Console.WriteLine(string.Format("StyleTopTextAttachment: {0}", (object) this.StyleTopTextAttachment));
      this.StyleBottomTextAttachmentType = (int) ba.ReadBS();
      Console.WriteLine(string.Format("StyleBottomTextAttachmentType: {0}", (object) this.StyleBottomTextAttachmentType));
      this.LeaderExtendedToText = ba.ReadB();
      Console.WriteLine(string.Format("LeaderExtendedToText: {0}", (object) this.LeaderExtendedToText));
      this.MLeaderAnnotContext.TextLabel = ba.ReadTU();
      Console.WriteLine("MLeaderAnnotContext.TextLabel: " + this.MLeaderAnnotContext.TextLabel);
      ba.DumpBitIndex();
      ba.bitindex += 17;
      ...
      foreach (LeaderRoot leaderRoot in this.MLeaderAnnotContext.LeaderRoots)
      {
        foreach (LeaderLine leaderLine in leaderRoot.LeaderLines)
        {
          leaderLine.LineTypeHandle = ba.ReadH();
          Console.WriteLine(string.Format("ll.LineTypeHandle: {0}", (object) leaderLine.LineTypeHandle));
          leaderLine.ArrowSymbolHandle = ba.ReadH();
          Console.WriteLine(string.Format("ll.ArrowSymbolHandle: {0}", (object) leaderLine.ArrowSymbolHandle));
        }
      }
      ...
      this.StyleBlockHandle = ba.ReadH();
      Console.WriteLine(string.Format("StyleBlockHandle: {0}", (object) this.StyleBlockHandle));
      ba.DumpBitIndex();
      Console.WriteLine("");
    }
  }
