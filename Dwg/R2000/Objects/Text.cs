...
// Type: dqgToPdf.Dwg.R2000.Objects.Text
...
{
  public class Text : dqgToPdf.Dwg.R2000.CommonEntityData, IText
  {
    public override void WritePdf(PageObject po, Dictionary<int, object> objMap = null)
    {
      ...
      if (this.StyleHandle != null)
      {
        ...
        if (targetHandle != 0 && objMap[targetHandle] is ShapeFile shapeFile)
        {
    ...
    public override void LinkInfoItem(LinkInfo li, Matrix matrix, Dictionary<int, object> objMap = null)
    {
      if (string.IsNullOrEmpty(this.LinkTargetPath))
        return;
      matrix = matrix.Clone();
      matrix.Translate((float) this.InsertionPt.x, (float) this.InsertionPt.y, MatrixOrder.Prepend);
      matrix.Rotate((float) (this.RotationAng / Math.PI * 180.0), MatrixOrder.Prepend);
